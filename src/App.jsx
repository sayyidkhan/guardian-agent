...
function useGuardian() {
  ...
  useEffect(() => {
    fetch("/api/profile")
      .then((res) => res.json())
      .then((data) => setGuardian(data.profile ?? null))
...
function Landing({ onStart }) {
  return (
    <section>
      ...
      <p>
        AI that keeps companies moving when humans are unavailable.
      </p>
...
function Setup({ initial, loading, onSaved }) {
...
function Ask({ guardian, loading }) {
  ...
  async function captureAndTranscribe() {
    ...
    try {
      ...
      setVoiceError(error.message || "Unable to record/transcribe audio.");
      setVoiceStatus("");
    } finally {
      if (stream) {
